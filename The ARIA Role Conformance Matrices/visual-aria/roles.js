		...
		check = function(nodes, obj, frames, focused, pNode, focusHidden){
			...
			(function(){
				...
				var calcNames = function(node){
					...
					var trim = function(str){
					...
					}, hasParentLabel = function(start, targ){
						if (start == targ)
							return false;
						...
						while (start){
							...
							if (start.getAttribute && start.getAttribute('aria-label'))
								return true;
						...
						return false;
					}, accName = '', accDesc = '', desc = '', aDescribedby = node.getAttribute('aria-describedby') || '',
